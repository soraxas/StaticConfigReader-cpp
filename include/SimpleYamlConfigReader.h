...
protected:
  ...
  template <typename F> _ConfigReaderRegisterClass(const F &callback) {
    callback.template __register<Args...>();
  }

public:
  static const _ConfigReaderRegisterClass &doRegister() { return instance; }
};

template <typename FileNameWithin64char,
          typename SecondFileNameWithin64char = void>
class StaticConfigReader {
public:
  template <typename Key, typename T> static const T &get() {
    _ConfigReaderRegisterClass<FileNameWithin64char, SecondFileNameWithin64char,
                               Key, T>::doRegister();
    static const T &thing = [] {
      ...
      try {
        ...
        std::string info = std::string() + "Trying to cast the key " +
                           Key::c_str() + " into type " + typeid(T).name();
  ...
  friend std::ostream &operator<<(std::ostream &_stream,
                                  const StaticConfigReader &t) {
    _stream << std::string(t);
    return _stream;
  }

  template <typename Key, typename T> static void __register() {
...
_ConfigReaderRegisterClass<File1, File2, Args...>
    _ConfigReaderRegisterClass<File1, File2, Args...>::instance{
